...
describe("NotificationRepository", () => {
  ...
  it("builds one placeholder per permission and maps recipient ids", async () => {
    ...
    expect(compactSql(query.mock.calls[0]?.[0])).toBe(
      "SELECT DISTINCT users.id FROM users INNER JOIN roles ON roles.id = users.role_id INNER JOIN role_permissions ON role_permissions.role_id = roles.id WHERE users.status = 'active' AND roles.is_active = 1 AND role_permissions.permission_id IN (?, ?)",
    );
